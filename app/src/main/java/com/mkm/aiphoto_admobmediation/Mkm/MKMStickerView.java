...
import com.mkm.aiphoto_admobmediation.Ultils.StickerUtils;
...
public class MKMStickerView extends RelativeLayout {
    ...
    private MKMStickerIcons currentIcon;
    ...
    private final List<MKMStickerIcons> icons;
    ...
    public MKMStickerView(Context context) {
        this(context, (AttributeSet) null);
    }

    public MKMStickerView(Context context, AttributeSet attributeSet) {
        this(context, attributeSet, 0);
    }

    @SuppressLint("ResourceType")
    public MKMStickerView(Context context, AttributeSet attributeSet, int i) {
    ...
    @RequiresApi(api = 21)
    public MKMStickerView(Context context, AttributeSet attributeSet, int i, int i2) {
    ...
    public void getStickerIcons() {
        MKMStickerIcons quShotStickerIconClose = new MKMStickerIcons(ContextCompat.getDrawable(getContext(), R.drawable.ic_outline_close), 0, MKMStickerIcons.DELETE);
        quShotStickerIconClose.setIconEvent(new DeleteIconEvent());
        MKMStickerIcons quShotStickerIconScale = new MKMStickerIcons(ContextCompat.getDrawable(getContext(), R.drawable.ic_outline_scale), 3, MKMStickerIcons.SCALE);
        quShotStickerIconScale.setIconEvent(new ZoomIconEvent());
        MKMStickerIcons quShotStickerIconFlip = new MKMStickerIcons(ContextCompat.getDrawable(getContext(), R.drawable.ic_outline_flip), 1, MKMStickerIcons.FLIP);
        quShotStickerIconFlip.setIconEvent(new FlipHorizontallyEvent());
        MKMStickerIcons quShotStickerIconEdit = new MKMStickerIcons(ContextCompat.getDrawable(getContext(), R.drawable.ic_outline_edit), 2, MKMStickerIcons.EDIT);
        ...
        if (this.handlingSticker != null && !this.locked && (this.showBorder || this.showIcons)) {
            ...
            if (this.showIcons) {
                ...
                while (i < this.icons.size()) {
                    MKMStickerIcons bitmapStickerIcon = this.icons.get(i);
                    switch (bitmapStickerIcon.getPosition()) {
                        case 0:
                            ...
                            break;
                        case 1:
                            if (((this.handlingSticker instanceof MKMTextView) && bitmapStickerIcon.getTag().equals(MKMStickerIcons.EDIT)) || ((this.handlingSticker instanceof DrawableSticker) && bitmapStickerIcon.getTag().equals(MKMStickerIcons.FLIP))) {
                                configIconMatrix(bitmapStickerIcon, f7, f8, calculateRotation);
                                bitmapStickerIcon.draw(canvas2, this.borderPaint);
                                break;
                            }
                        case 2:
                            if (this.handlingSticker instanceof MKMSticker) {
                                if (((MKMSticker) this.handlingSticker).getType() != 0) {
                                    break;
                                } else {
                        ...
                        case 3:
                            if ((!(this.handlingSticker instanceof MKMTextView) || !bitmapStickerIcon.getTag().equals(MKMStickerIcons.ROTATE)) && (!(this.handlingSticker instanceof DrawableSticker) || !bitmapStickerIcon.getTag().equals(MKMStickerIcons.SCALE))) {
                                if (this.handlingSticker instanceof MKMSticker) {
                                    MKMSticker beautySticker = (MKMSticker) this.handlingSticker;
    ...
    public void configIconMatrix(@NonNull MKMStickerIcons bitmapStickerIcon, float f, float f2, float f3) {
    ...
    public void zoomAndRotateSticker(@Nullable Sticker sticker, @NonNull MotionEvent motionEvent) {
        float f;
        if (sticker != null) {
            boolean z = sticker instanceof MKMSticker;
            if (z) {
                MKMSticker beautySticker = (MKMSticker) sticker;
                if (beautySticker.getType() == 10 || beautySticker.getType() == 11) {
                    return;
                }
            }
    ...
    @Nullable
    public MKMStickerIcons findCurrentIconTouched() {
        for (MKMStickerIcons next : this.icons) {
    ...
    @NonNull
    public MKMStickerView addSticker(@NonNull Sticker sticker) {
        return addSticker(sticker, 1);
    }

    public MKMStickerView addSticker(@NonNull final Sticker sticker, final int i) {
        if (ViewCompat.isLaidOut(this)) {
            addStickerImmediately(sticker, i);
        } else {
            post(new Runnable() {
                public void run() {
                    MKMStickerView.this.addStickerImmediately(sticker, i);
                }
            });
        }
        return this;
    ...
    public void setStickerPosition(@NonNull Sticker sticker, int i) {
        ...
        if (sticker instanceof MKMSticker) {
            MKMSticker beautySticker = (MKMSticker) sticker;
            f = height / 2.0f;
    ...
    @NonNull
    public MKMStickerView setLocked(boolean z) {
        this.locked = z;
        invalidate();
        return this;
    }
    ...
    @NonNull
    public MKMStickerView setMinClickDelayTime(int i) {
    ...
    @NonNull
    public MKMStickerView setConstrained(boolean z) {
        this.constrained = z;
        postInvalidate();
        return this;
    }
    ...
    @NonNull
    public MKMStickerView setOnStickerOperationListener(@Nullable OnStickerOperationListener onStickerOperationListener2) {
    ...
    @NonNull
    public List<MKMStickerIcons> getIcons() {
        return this.icons;
    }

    public void setIcons(@NonNull List<MKMStickerIcons> list) {
        this.icons.clear();
