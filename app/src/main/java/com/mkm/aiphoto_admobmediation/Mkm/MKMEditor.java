...
import com.mkm.aiphoto_admobmediation.Draw.OnSaveBitmap;
import com.mkm.aiphoto_admobmediation.Listener.BrushColorChangeListener;
...
public class MKMEditor implements BrushColorChangeListener {
    ...
    private OnMKMEditorListener mOnPhotoEditorListener;
    public MKMView parentView;
    ...
    private MKMEditor(Builder builder) {
...
//      if (!str.equals(""))
    ...
    public void saveAsFile(@NonNull String str, @NonNull OnSaveListener onSaveListener) {
        saveAsFile(str, new MKMSettings.Builder().build(), onSaveListener);
    }

    @RequiresPermission(allOf = {"android.permission.WRITE_EXTERNAL_STORAGE"})
    @SuppressLint({"StaticFieldLeak"})
    public void saveAsFile(@NonNull final String str, @NonNull final MKMSettings saveSettings, @NonNull final OnSaveListener onSaveListener) {
                ...
                new AsyncTask<String, String, Exception>() {
                    ...
                    public Exception doInBackground(String... strArr) {
                        ...
                        try {
                            ...
                            if (MKMEditor.this.parentView != null) {
                                if (saveSettings.isTransparencyEnabled()) {
                                    bitmap = MKMBitmapUtils.removeTransparency(getBitmapFromView(MKMEditor.this.parentView));
                                } else {
                                    bitmap = getBitmapFromView(MKMEditor.this.parentView);
                                }
                                bitmap.compress(Bitmap.CompressFormat.PNG, 100, fileOutputStream);
                            }
                            fileOutputStream.flush();
                            fileOutputStream.close();
                            Log.d(MKMEditor.TAG, "Filed Saved Successfully");
                            return null;
                        } catch (Exception e) {
                            e.printStackTrace();
                            Log.d(MKMEditor.TAG, "Failed to save File");
                            return e;
                        }
                    }
    ...
    public void saveStickerAsBitmap(@NonNull OnSaveBitmap onSaveBitmap) {
        saveStickerAsBitmap(new MKMSettings.Builder().build(), onSaveBitmap);
    }

    @SuppressLint({"StaticFieldLeak"})
    public void saveStickerAsBitmap(@NonNull MKMSettings saveSettings, @NonNull OnSaveBitmap onSaveBitmap) {
        Bitmap bitmap;

        if (saveSettings.isTransparencyEnabled()) {
            bitmap = MKMBitmapUtils.removeTransparency(getBitmapFromView(this.parentView));
        } else {
    ...
    public void setOnPhotoEditorListener(@NonNull OnMKMEditorListener onPhotoEditorListener) {
    ...
    public static class Builder {
        ...
        public Builder(Context context2, MKMView polishView) {
        ...
        public MKMEditor build() {
            return new MKMEditor(this);
        }
    }
